...
output_path_toilet = './XX_Data/Plumbingandelectric_toilet-' + str(fecha.year) + '_' + str(fecha.month) + '.csv'

# Path for loading the URL sites
url_path_toilet = './XX_Url/Plumbingandelectric.xlsx'
...
def plumblingandelectric_data(elem, soup_html):
    """
    ...
    information = [datetime.datetime.today().date(), elem["Fabricante"], elem["Sku"],
                   ...
                   price_clean, "USD", "plumbingandelectric.com", stock, elem["Link"], url_img]
...
for product_type, output_path in [[url_path_toilet, output_path_toilet]]:
    ...
    for index, elem in product_df.iterrows():
        ...
        if estate == 200:
            ...
            toilet_information = plumblingandelectric_data(elem, soup)
