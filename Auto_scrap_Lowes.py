...
def lowes_data(elem, data):
    ...
    information = [datetime.datetime.today().date(), brand_name, elem["Linea"], product_format, elem["Rough in"],
                   elem["Bowl Height"], elem["Asiento / Capacidad"], product_name, sku_ref, item_ref,
                   price_clean, "USD", "lowes.com", stock, elem["Link"], url_img]
...
for product_type, output_path in [[url_path_toilet, output_path_toilet]]:
    ...
    df = pd.DataFrame(datos, columns=["Fecha", "Marca", "Linea", "Tipo", "Rough_In", "Bowl Height", "Asiento/Capacidad",
